        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
          {stats.map((stat, i) => (
            <ScrollReveal key={stat.label} delay={i * 0.1} className="text-center">
              <div className="font-mono text-2xl sm:text-4xl md:text-5xl font-bold text-green-400">
                <CountUp end={stat.value} suffix={stat.suffix} duration={2.5} />
              </div>
              <p className="text-neutral-500 text-xs sm:text-sm mt-2 uppercase tracking-wide sm:tracking-wider">{stat.label}</p>
            </ScrollReveal>
          ))}
        </div>
      </div>
