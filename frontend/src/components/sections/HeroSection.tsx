...
export function HeroSection() {
  ...
  useGSAP((context, contextSafe) => {
    ...
    // Bar
    tl.fromTo("[data-hero-bar]", { scaleX: 0, opacity: 0 }, { scaleX: 1, opacity: 1, duration: 0.6 })

    // Show wrapper
    tl.set("[data-hero-line]", { opacity: 1 }, 0.1)

    // Letters — blur + fade + rise
    tl.fromTo(
      "[data-hero-letter]",
      { opacity: 0, y: 30, filter: "blur(8px)" },
      { opacity: 1, y: 0, filter: "blur(0px)", duration: 0.8, stagger: 0.07, ease: "power2.out" },
      0.2
    )
    ...
    // Images — target only parallax images
    tl.fromTo(
      "[data-hero-img]",
      { opacity: 0, scale: 0.9 },
    ...
    letters.forEach((letter) => {
      ...
      const onLeave = contextSafe!(() => {
        const isGreenK = letter.classList.contains("text-green-400")
        gsap.to(letter, {
          scale: 1,
          y: 0,
          zIndex: 0,
          color: isGreenK ? "#4ade80" : "#ffffff",
          duration: 0.4,
          ease: "power2.inOut",
          overwrite: "auto",
        })
    ...
    >
          ...
          <h1 className="text-6xl md:text-8xl lg:text-9xl text-white font-bold tracking-tighter uppercase">
            {"Клякса".split("").map((char, i) => (
              <span
                key={i}
                data-hero-letter
                className={`inline-block cursor-default select-none relative ${char === "к" && i === 3 ? "text-green-400" : ""}`}
      ...
      <Floating sensitivity={-1} className="overflow-hidden hidden md:block">
        ...
        <FloatingElement depth={1} className="top-[16%] left-[32%]">
          <img data-hero-img style={{ opacity: 0 }} src={tattooImages[1].url} alt={tattooImages[1].title}
        ...
        <FloatingElement depth={2} className="top-[8%] left-[48%]">
          <img data-hero-img style={{ opacity: 0 }} src={tattooImages[2].url} alt={tattooImages[2].title}
        ...
        <FloatingElement depth={1} className="top-[14%] left-[88%]">
          <img data-hero-img style={{ opacity: 0 }} src={tattooImages[3].url} alt={tattooImages[3].title}
        ...
        <FloatingElement depth={1} className="top-[40%] left-[2%]">
          <img data-hero-img style={{ opacity: 0 }} src={tattooImages[4].url} alt={tattooImages[4].title}
        ...
        <FloatingElement depth={2} className="top-[65%] left-[75%]">
          <img data-hero-img style={{ opacity: 0 }} src={tattooImages[5].url} alt={tattooImages[5].title}
        ...
        <FloatingElement depth={4} className="top-[73%] left-[15%]">
          <img data-hero-img style={{ opacity: 0 }} src={tattooImages[6].url} alt={tattooImages[6].title}
        ...
        <FloatingElement depth={1} className="top-[80%] left-[50%]">
          <img data-hero-img style={{ opacity: 0 }} src={tattooImages[7].url} alt={tattooImages[7].title}
            className="w-24 h-24 md:w-32 md:h-32 object-cover hover:scale-105 duration-200 cursor-pointer transition-transform rounded-lg border border-green-500/20" />
        </FloatingElement>

        {/* Новые фотографии */}
        <FloatingElement depth={1.5} className="top-[35%] left-[90%]">
          <img data-hero-img style={{ opacity: 0 }} src={tattooImages[8].url} alt={tattooImages[8].title}
            className="w-20 h-20 md:w-28 md:h-28 object-cover hover:scale-105 duration-200 cursor-pointer transition-transform rounded-lg border border-green-500/20" />
        </FloatingElement>
        <FloatingElement depth={2} className="top-[50%] left-[18%]">
          <img data-hero-img style={{ opacity: 0 }} src={tattooImages[9].url} alt={tattooImages[9].title}
            className="w-24 h-24 md:w-32 md:h-32 object-cover hover:scale-105 duration-200 cursor-pointer transition-transform rounded-lg border border-green-500/20" />
        </FloatingElement>
        <FloatingElement depth={0.8} className="top-[50%] left-[72%]">
          <img data-hero-img style={{ opacity: 0 }} src={tattooImages[10].url} alt={tattooImages[10].title}
        ...
        <FloatingElement depth={3} className="top-[85%] left-[30%]">
          <img data-hero-img style={{ opacity: 0 }} src={tattooImages[11].url} alt={tattooImages[11].title}
            className="w-24 h-24 md:w-32 md:h-32 object-cover hover:scale-105 duration-200 cursor-pointer transition-transform rounded-lg border border-green-500/20" />
        </FloatingElement>
        <FloatingElement depth={1.2} className="top-[28%] left-[72%]">
          <img data-hero-img style={{ opacity: 0 }} src={tattooImages[12].url} alt={tattooImages[12].title}
            className="w-16 h-16 md:w-24 md:h-24 object-cover hover:scale-105 duration-200 cursor-pointer transition-transform rounded-lg border border-green-500/20" />
        </FloatingElement>
        <FloatingElement depth={2.5} className="top-[85%] left-[88%]">
          <img data-hero-img style={{ opacity: 0 }} src={tattooImages[13].url} alt={tattooImages[13].title}
            className="w-20 h-20 md:w-28 md:h-28 object-cover hover:scale-105 duration-200 cursor-pointer transition-transform rounded-lg border border-green-500/20" />
        </FloatingElement>
        <FloatingElement depth={1.8} className="top-[8%] left-[68%]">
          <img data-hero-img style={{ opacity: 0 }} src={tattooImages[14].url} alt={tattooImages[14].title}
